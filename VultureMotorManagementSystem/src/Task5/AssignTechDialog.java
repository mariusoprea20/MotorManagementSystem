...
public class AssignTechDialog extends JDialog {
	 ...
	 */
	private JobProgressApp app; // declared to user TaskAllocationApp from DB
	private int taskID; // retrieve the task id from TaskAllocationComponent Class
	private int techID;  // assign selected Tech ID from this class table when pressing OK JButton


	/*** Create the dialog constructor***/
	public AssignTechDialog(JobProgressApp app, int taskID, TaskGUI task) {
		// connect TaskAllocationApp here to use it's functions
		this.app=app;
		this.taskID=taskID;
		setTitle("List of Technicians");
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if(selectedRow>=0) {
						 ...
						 app.database.updateTech(taskID, techID);//update the database
						 task.displayTableData(app.database.getAllTasks()); // display the data
		...
		this.displayTableData(app.database.getAllTech());
	}
