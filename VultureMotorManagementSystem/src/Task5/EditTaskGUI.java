...
public class EditTaskGUI extends JFrame {
	...
	public EditTaskGUI(JobProgressApp app, TaskGUI taskTable, int taskID) {
		this.app=app;
		...
		editBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// retrieve the text from the text fields and date chooser fields
				...
				try {
					...
					//edit task and refresh the JTable in TaskGUI class
					app.database.editTaskDetails(taskType, taskStatus, startDate, endDate, deadline, desc, notes, taskID, (int)days);
		...
		//display the tasks in the required fields
		displayTaskDetails(taskID);
	}
	
	
	//retrieve all details about the task and set the fields to it's corresponding details
	public void displayTaskDetails(int taskID) {

		for(Task task:app.database.getAllTasks()) {
			if(task.getTaskId()==taskID) {
			    taskTypeFld.setText(task.getTask_type());
				descTextArea.setText(task.getTask_desc());
				notesTextArea.setText(task.getNotes());
			    ((JTextField)startDateChooser.getDateEditor().getUiComponent()).setText(task.getTask_startDate());
				((JTextField)endDateChooser.getDateEditor().getUiComponent()).setText(task.getTask_endDate());
				((JTextField)deadlineDateChooser.getDateEditor().getUiComponent()).setText(task.getTask_deadline().toString());
				statusComboBox.setSelectedItem(task.getTask_status());
			}
		}
	}
}
